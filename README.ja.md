...
> 初回起動はコンテナイメージのビルド・取得と2つのローカルモデル — LLM（約9.6GB）と埋め込みモデル（約2.2GB）、合計約12GB — のダウンロードを含みます。所要時間は回線速度に依存し、高速回線で約10〜15分（埋め込みモデルのダウンロードが最も遅い区間になりがちです）、低速回線ではそれ以上かかります。以降はキャッシュされます。
