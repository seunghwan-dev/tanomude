...
REVISE_MARKER = "【修正指示】"
GROUNDED_SLOT_LABELS = {"dest_code": "目的地", "purpose": "目的"}
REVISE_BLOCKED_NOTICE = (
    "申し訳ありません。{item}は指示文で確定する項目のため、修正では変更できません。"
...
def base_instruction(instruction: str) -> str:
    return instruction.split(REVISE_MARKER, 1)[0]
...
def immune_extractor(rag_context: str, extractor: SlotExtractor = extract_slots) -> SlotExtractor:
    def slot_fn(request: RequestInput, corrected_context: str) -> Slots:
        ...
        grounded = extractor(request.model_copy(update={"instruction": base}), rag_context)
        if corrected_context == rag_context:
            return grounded
        corrected = extractor(request, corrected_context)
        return enforce_immunity(grounded, corrected, base)

    return slot_fn


class ReviseAssessment(NamedTuple):
    persist: bool
    blocked_slot: str | None


def revise_changes_movable_slot(grounded: Slots, corrected: Slots, instruction: str) -> bool:
    if grounded.reuse_prev_proj != corrected.reuse_prev_proj:
        return True
    return not instruction_grounds_overseas(instruction) and grounded.overseas != corrected.overseas


def revise_blocked_slot(grounded: Slots, corrected: Slots, instruction: str) -> str | None:
    if revise_changes_movable_slot(grounded, corrected, instruction):
        return None
    if grounded.dest_code.upper() != corrected.dest_code.upper():
        return GROUNDED_SLOT_LABELS["dest_code"]
    if grounded.purpose != corrected.purpose:
        return GROUNDED_SLOT_LABELS["purpose"]
    return None


def revise_assessment(grounded: Slots, corrected: Slots, instruction: str) -> ReviseAssessment:
    return ReviseAssessment(
        persist=revise_changes_movable_slot(grounded, corrected, instruction),
        blocked_slot=revise_blocked_slot(grounded, corrected, instruction),
    )


def revise_blocked_notice(slot_label: str) -> str:
    return REVISE_BLOCKED_NOTICE.format(item=slot_label)
